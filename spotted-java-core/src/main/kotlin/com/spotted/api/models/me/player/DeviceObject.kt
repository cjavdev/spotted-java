...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun isPrivateSession(): Optional<Boolean> = isPrivateSession.getOptional("is_private_session")
     ...
     *   server responded with an unexpected value).
     */
    fun isRestricted(): Optional<Boolean> = isRestricted.getOptional("is_restricted")
     ...
     *   server responded with an unexpected value).
     */
    fun supportsVolume(): Optional<Boolean> = supportsVolume.getOptional("supports_volume")
     ...
     *   server responded with an unexpected value).
     */
    fun volumePercent(): Optional<Long> = volumePercent.getOptional("volume_percent")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.volumePercent] with `volumePercent.orElse(null)`. */
        fun volumePercent(volumePercent: Optional<Long>) = volumePercent(volumePercent.getOrNull())
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            ...
            (if (isPrivateSession.asKnown().isPresent) 1 else 0) +
            (if (isRestricted.asKnown().isPresent) 1 else 0) +
            (if (name.asKnown().isPresent) 1 else 0) +
            (if (supportsVolume.asKnown().isPresent) 1 else 0) +
            (if (type.asKnown().isPresent) 1 else 0) +
            (if (volumePercent.asKnown().isPresent) 1 else 0)
