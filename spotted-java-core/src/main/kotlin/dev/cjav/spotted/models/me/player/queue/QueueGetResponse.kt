...
private constructor(
     ...
     */
    fun currentlyPlaying(): Optional<CurrentlyPlaying> =
        currentlyPlaying.getOptional("currently_playing")
     ...
     *   server responded with an unexpected value).
     */
    fun queue(): Optional<List<Queue>> = queue.getOptional("queue")
    ...
    fun validate(): QueueGetResponse = apply {
        ...
        currentlyPlaying().ifPresent { it.validate() }
        published()
        queue().ifPresent { it.forEach { it.validate() } }
        validated = true
    }
    ...
    internal fun validity(): Int =
        (currentlyPlaying.asKnown().getOrNull()?.validity() ?: 0) +
            (if (published.asKnown().isPresent) 1 else 0) +
            (queue.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
    ...
    private constructor(
         ...
         * import java.util.Optional;
         *
         * Optional<String> result = currentlyPlaying.accept(new CurrentlyPlaying.Visitor<Optional<String>>() {
         *     @Override
        ...
        companion object {
            ...
            @JvmStatic fun ofTrack(track: TrackObject) = CurrentlyPlaying(track = track)

            @JvmStatic fun ofEpisode(episode: EpisodeObject) = CurrentlyPlaying(episode = episode)
        }
    ...
    private constructor(
         ...
         * import java.util.Optional;
         *
         * Optional<String> result = queue.accept(new Queue.Visitor<Optional<String>>() {
         *     @Override
         *     public Optional<String> visitTrack(TrackObject track) {
         *         return Optional.of(track.toString());
         *     }
        ...
        companion object {
            ...
            @JvmStatic fun ofTrack(track: TrackObject) = Queue(track = track)

            @JvmStatic fun ofEpisode(episode: EpisodeObject) = Queue(episode = episode)
        }
