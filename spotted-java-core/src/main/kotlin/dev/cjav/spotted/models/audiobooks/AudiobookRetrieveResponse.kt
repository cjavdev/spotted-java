...
class AudiobookRetrieveResponse
...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [AudiobookRetrieveResponse].
         *
    ...
    /** A builder for [AudiobookRetrieveResponse]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(audiobookRetrieveResponse: AudiobookRetrieveResponse) = apply {
            id = audiobookRetrieveResponse.id
            authors = audiobookRetrieveResponse.authors.map { it.toMutableList() }
            availableMarkets = audiobookRetrieveResponse.availableMarkets.map { it.toMutableList() }
            copyrights = audiobookRetrieveResponse.copyrights.map { it.toMutableList() }
            ...
            images = audiobookRetrieveResponse.images.map { it.toMutableList() }
            languages = audiobookRetrieveResponse.languages.map { it.toMutableList() }
            mediaType = audiobookRetrieveResponse.mediaType
            name = audiobookRetrieveResponse.name
            narrators = audiobookRetrieveResponse.narrators.map { it.toMutableList() }
            ...
            additionalProperties = audiobookRetrieveResponse.additionalProperties.toMutableMap()
        }
         ...
         *
         * You should usually call [Builder.chapters] with a well-typed [Chapters] value instead.
         * This method is primarily for setting the field to an undocumented or not yet supported
         * value.
         */
        ...
        /**
         * Returns an immutable instance of [AudiobookRetrieveResponse].
         *
         ...
         */
        fun build(): AudiobookRetrieveResponse =
    ...
    fun validate(): AudiobookRetrieveResponse = apply {
        if (validated) {
            return@apply
        }
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            (authors.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
    ...
    private constructor(
        ...
        class Builder internal constructor() {
             ...
             * `List<SimplifiedChapterObject>` value instead. This method is primarily for setting
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AudiobookRetrieveResponse &&
            id == other.id &&
