...
private constructor(
     ...
     */
    fun artists(): Optional<List<SimplifiedArtistObject>> = artists.getOptional("artists")
     ...
     *   server responded with an unexpected value).
     */
    fun copyrights(): Optional<List<CopyrightObject>> = copyrights.getOptional("copyrights")
     ...
     */
    @Deprecated("deprecated")
    fun externalIds(): Optional<ExternalIdObject> = externalIds.getOptional("external_ids")
     ...
     *   server responded with an unexpected value).
     */
    @Deprecated("deprecated") fun label(): Optional<String> = label.getOptional("label")
     ...
     *   server responded with an unexpected value).
     */
    fun restrictions(): Optional<AlbumRestrictionObject> = restrictions.getOptional("restrictions")
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [AlbumRetrieveResponse].
         *
    ...
    /** A builder for [AlbumRetrieveResponse]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(albumRetrieveResponse: AlbumRetrieveResponse) = apply {
            id = albumRetrieveResponse.id
            albumType = albumRetrieveResponse.albumType
            availableMarkets = albumRetrieveResponse.availableMarkets.map { it.toMutableList() }
            externalUrls = albumRetrieveResponse.externalUrls
            href = albumRetrieveResponse.href
            images = albumRetrieveResponse.images.map { it.toMutableList() }
            name = albumRetrieveResponse.name
            releaseDate = albumRetrieveResponse.releaseDate
            releaseDatePrecision = albumRetrieveResponse.releaseDatePrecision
            totalTracks = albumRetrieveResponse.totalTracks
            type = albumRetrieveResponse.type
            uri = albumRetrieveResponse.uri
            artists = albumRetrieveResponse.artists.map { it.toMutableList() }
            copyrights = albumRetrieveResponse.copyrights.map { it.toMutableList() }
            externalIds = albumRetrieveResponse.externalIds
            genres = albumRetrieveResponse.genres.map { it.toMutableList() }
            label = albumRetrieveResponse.label
            popularity = albumRetrieveResponse.popularity
            published = albumRetrieveResponse.published
            restrictions = albumRetrieveResponse.restrictions
            tracks = albumRetrieveResponse.tracks
            additionalProperties = albumRetrieveResponse.additionalProperties.toMutableMap()
        }
         ...
         * method is primarily for setting the field to an undocumented or not yet supported value.
         */
        @Deprecated("deprecated") fun label(label: JsonField<String>) = apply { this.label = label }
        ...
        /**
         * Returns an immutable instance of [AlbumRetrieveResponse].
         *
         ...
         */
        fun build(): AlbumRetrieveResponse =
    ...
    fun validate(): AlbumRetrieveResponse = apply {
        if (validated) {
            return@apply
        }
        ...
        uri()
        artists().ifPresent { it.forEach { it.validate() } }
        copyrights().ifPresent { it.forEach { it.validate() } }
        externalIds().ifPresent { it.validate() }
        genres()
        label()
        popularity()
        published()
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            (albumType.asKnown().getOrNull()?.validity() ?: 0) +
            ...
            (if (totalTracks.asKnown().isPresent) 1 else 0) +
            type.let { if (it == JsonValue.from("album")) 1 else 0 } +
            (if (uri.asKnown().isPresent) 1 else 0) +
            (artists.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
            (copyrights.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
            (externalIds.asKnown().getOrNull()?.validity() ?: 0) +
            (genres.asKnown().getOrNull()?.size ?: 0) +
            (if (label.asKnown().isPresent) 1 else 0) +
            (if (popularity.asKnown().isPresent) 1 else 0) +
    ...
    class AlbumType @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            @JvmField val ALBUM = of("album")

            @JvmField val SINGLE = of("single")

            @JvmField val COMPILATION = of("compilation")

            @JvmStatic fun of(value: String) = AlbumType(JsonField.of(value))
        }
    ...
    private constructor(
         ...
         *   server responded with an unexpected value).
         */
        fun items(): Optional<List<SimplifiedTrackObject>> = items.getOptional("items")
        ...
        class Builder internal constructor() {
             ...
             * `List<SimplifiedTrackObject>` value instead. This method is primarily for setting the
             * field to an undocumented or not yet supported value.
             */
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AlbumRetrieveResponse &&
            id == other.id &&
