...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { SpottedInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw SpottedInvalidDataException("`$name` is not set")
            is JsonNull -> throw SpottedInvalidDataException("`$name` is null")
            else -> throw SpottedInvalidDataException("`$name` is invalid, received $this")
        }
